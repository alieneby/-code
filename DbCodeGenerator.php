<?php
    ...
    class DbCodeGenerator
    {
        ...
        function getDBStructure( $strDatabase ) {
            ...
            $result = mysqli_query( AlienDB::$_linkId, 'SELECT @@GLOBAL.sql_mode global, @@SESSION.sql_mode session' );
            if ( ! $result ) {
                return Log::error( 'Error getting sql_mode: ' . mysqli_error(), __METHOD__, __LINE__, __FILE__ );
            }
            if ( $row = mysqli_fetch_row( $result ) ) {
                $strSqlMode = $row[ 0 ] . '/' . $row[ 1 ];
                if ( stripos( $strSqlMode, 'NO_ZERO_IN_DATE' ) !== false || stripos( $strSqlMode, 'NO_ZERO_DATE' ) !== false ) {
                    $this->_fNoZeroDate = true;
                }
            }
        ...
        function getGeneratedDefaultValue( $codeGenColumn ) {
            ...
            if ( isset( $codeGenColumn->_strDefault ) ) {
                ...
                } elseif ( strtoupper( $codeGenColumn->_strDefault ) == 'NULL' ) {
                    return array( 'null', "null" );
                } else {
                    return array( "'" . $codeGenColumn->_strDefault . "'", "'" . $codeGenColumn->_strDefault . "'" );
                }
...
';
            ...
            foreach ( $tb->_arrColumns as $codeGenColumn ) {
                ...
                } elseif ( $t == "int" || $t == "tinyint" || $t == "decimal" || $t == "smallint" || $t == "bigint" ) {
            ...
            $this->code[] = '';
            $this->code[] = '    /**';
            $this->code[] = '    * @param $strWhere string            SQL Where part without where. Eg. "id>10 order by id limit 1,2"';
            $this->code[] = '    * @param $fCopy boolean              Store copy? Needed in update, to set only changed values.';
            $this->code[] = '    * @param $strSelectColumns string    Comma separated column names.';
            $this->code[] = '    * @return ' . $tb->_strNamePhp . '[]';
            $this->code[] = '    */';
